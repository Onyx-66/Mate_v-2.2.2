...
def show():
    ...
    # Engine is hardcoded to Deepseek
    engine_code = "deepseek"
    engine_env_var = "DEEPSEEK_API_KEY"
    st.session_state.engine_code = engine_code

    # Check API key silently or show error if missing
    if not check_api_key(engine_env_var):
        st.error("⚠️ Deepseek API key is missing. Please check your .env file.")
    ...
    if st.button("🚀 Generate study pack", type="primary", use_container_width=True):
        if not subject.strip() or not chapter.strip():
            ...
            st.error("⚠️ Deepseek API key is missing.")
        else:
